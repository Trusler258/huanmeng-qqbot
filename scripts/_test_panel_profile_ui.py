...
async def main():
    ...
    async with async_playwright() as p:
        ...
        #    所以前端给画像页加了 `.profile-tab` 作为稳定钩子。
        PANE = ".profile-tab"

        async def body_rows(pane=PANE):
            rows = await page.query_selector_all(pane + " .arco-table-tr")
            out = []
            for r in rows:
        ...
        ck("不再是好友度表的残留行", "| 存 | 删" not in first, first[:60])
        ...
        if priv:
            ...
            ck("私聊筛选后只剩私聊", all("私聊" in t for t in texts) and len(texts) > 0,
               "%d 行" % len(texts))
