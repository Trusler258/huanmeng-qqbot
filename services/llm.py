...
def _build_messages(
    ...
    fmt_reminder = (
        ...
        "★ 笔记本规则（必须真记，出现即调）：①群内关系（谁和谁什么关系）②称呼/黑话（某外号指谁、某词在群里什么意思）③某人的身份/昵称/性别/特征 ④有人明确说\"记一下/记住\"。碰到任一类，立即用 calls 调 note 记录，如 {\"name\":\"note\",\"args\":\"模组东=雪泠翎，女生\"}。写清楚\"谁=什么信息\"或\"谁和谁=什么关系\"。**判定要点：只要群里的人这样称呼/这样认为就照记——哪怕是知名角色或公众人物，你记的是「这个群的事实」，不是百科。**只在回复里说\"记下了\"而没有实际调用，是骗人，绝对禁止。\n"
...
async def generate_multi_reply(
    ...
    fmt_reminder = (
        ...
        f"回复长度随场景：闲聊 1~3 句、每句≤{max_chars}字；知识/技术/原理/概念/步骤/对比类问题或对方追问 → 3~10 句、不限字数，讲深讲透。fav -5~+5。"
            ...
            "\n"
            '【笔记本规则（必须真记，出现即调）：①群内关系 → ②称呼/黑话（某外号指谁、某词群内什么意思）→ ③某人身份/昵称/性别/特征 → ④有人明确说"记一下/记住"。碰到任一类，立即用 calls 调 note 记录，如 {"name":"note","args":"模组东=雪泠翎，女生"}，写清"谁=什么信息"。**判定要点：只要群里的人这样称呼/这样认为就照记（哪怕是知名角色/公众人物）——你记的是「这个群的事实」，不是百科。**只在回复说"记下了"而没调用是骗人。】'
            "\n"
