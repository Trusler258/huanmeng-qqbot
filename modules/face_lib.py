...
# 主词 → 一句话画面简述（v2.3.17）
# 背景：只给词表时 LLM 全靠猜"得意长什么样"，实测 mood=得意 却配了疑问图。
# 这里的描述来自 data/faces_tag_corrected.md 的逐张校对结论（同批素材同一画面风格），
# 让 LLM 把情绪词和真实画面对上号，选图不再盲猜。
# 48 个主词全覆盖；素材换批时同步改这张表（否则描述会骗人）。
_FACE_DESC: dict[str, str] = {
    "开心": "眯眼笑", "坏笑": "抿嘴偷笑不怀好意", "得意": "抬下巴嘚瑟",
    "害羞": "脸红捂脸", "大哭": "眼泪哗哗", "委屈": "瘪嘴要哭",
    "失落": "低头丧气", "生气": "鼓脸瞪眼", "不满": "嘟嘴",
    "慌张": "手忙脚乱冒汗", "大喊": "张大嘴喊", "惊讶": "瞪圆眼",
    "疑惑": "歪头冒问号", "无聊": "瘫着没精神", "疲惫": "累瘫叹气",
    "不舍": "恋恋不舍回头", "道歉": "低头认错", "祝贺": "举杯庆祝",
    "加油": "攥拳打气", "拒绝": "摆手不要", "饿": "眼冒食物",
    "闭眼": "安静闭目", "睡觉": "睡着了", "困": "揉眼打瞌睡",
    "打哈欠": "张嘴哈欠", "发呆": "放空走神", "打招呼": "举手问好",
    "挥手": "摆手拜拜", "眨眼": "单眼wink", "抱猫": "抱着猫",
    "抱玩偶": "抱玩偶", "抱蛋糕": "捧小蛋糕", "抱爱心": "抱大爱心",
    "抱花": "捧花束", "抱东西": "抱着纸袋", "端东西": "端茶盘",
    "吃东西": "塞满嘴", "吃冰淇淋": "舔冰淇淋", "喝饮料": "叼吸管",
    "逗猫": "拿逗猫棒", "敬礼": "标准敬礼", "没问题": "比OK手势",
    "蹦跳": "原地起跳", "偷看": "探头张望", "张望": "左右看",
    "站着": "呆站", "坐着": "乖乖坐", "趴着": "瘫成一滩",
}
...
def face_keywords_hint() -> str:
    ...
    v2.3.17: 词表 → 「词(画面简述)」富提示。LLM 看得懂每个词的实际画面，
    mood 和 [FACE:] 才能对上（此前 mood=得意 配疑问图，因为 LLM 不知道词义画面）。
    示例输出: 开心(眯眼笑)/坏笑(抿嘴偷笑不怀好意)/得意(抬下巴嘚瑟)/...
    """
    ...
    for w in ws:
        d = _FACE_DESC.get(w)
        parts.append(f"{w}({d})" if d else w)
    return "/".join(parts)
