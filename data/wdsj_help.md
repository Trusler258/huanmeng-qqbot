...
`/~wdsj <模式> <玩家>`（默认发图片卡片；结尾加 `text` 看文字版）
...
`/~wdsj me`  或  `/~wdsj <玩家名> me`

一张横屏图同时展示 **起床战争（33 项）+ 竞技场（17 项）** 全部字段，每个字段配 Minecraft 原版图标，底部另有 18 项衍生比率指标。

- `/~wdsj me` —— 用你绑定的玩家名（需先 `/~wdsj bd <玩家名>`）
- `/~wdsj dongtians me` —— 指定玩家名，不需要绑定
