...
### 四、附带发现（已确认，未改动，待定）
1. **`执行reward` 不带 `/~` 前缀不会执行**：用户发的是「执行reward」，缺前缀 → 不识别为指令
   → 交给 LLM → LLM 猜错意图调了 `system_status` 工具（查 PC 状态）→ 拿到"PC 状态空"
   → 把赞赏码和 PC 状态串在一起瞎编。
   **指令本身完全正常**：`data/images/reward_qrcode.png` 存在（3693 字节），
   实跑 `cmd_reward([])` 返回正确的 CQ 码。
2. **`||` 已是废弃机制**：主回复用 JSON `replies` 数组表达多句；提示词里 `||` 只作为
   **反例**出现（`10_format_group.md`、`40_reminders.md` 的"禁止这样写"），
   `config/update.toml` 里的"用 ||| 分隔"是 v1.x 的历史日志。
   `_clean_sentences` 的 `||` 分支保留作兼容兜底（LLM 偶尔误用时能正确拆开而非泄漏）。
3. **msglog 重复记录**：`msglog_247478659.jsonl` 中 bot 消息 719 条，按
   (msg_id, 内容) 去重后 625 条，**94 条完全重复**。日志显示实际只发送一次
   （`send_group_msg` 成功 1 次），说明是记录环节重复写，会影响记忆/统计/画像计数。
